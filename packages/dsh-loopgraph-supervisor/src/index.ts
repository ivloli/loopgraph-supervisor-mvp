...
function setState(agent: Agent, state: LoopState): void {
  states.set(agent, state)
}

function emitDecision(agent: Agent, state: LoopState, record: ReturnType<typeof decision>): LoopState {
...
function transition(agent: Agent, state: LoopState, next: Partial<LoopState>, reason: string): LoopState {
...
export function specRevision(goal: string, maxAttempts: number, acceptance: AcceptanceContract): string {
  return createHash('sha256').update(canonical({ goal, maxAttempts, acceptance })).digest('hex')
}

function isCurrentExecution(agent: Agent, expected: LoopState, node: LoopState['node']): boolean {
...
async function applyToCurrentExecution(agent: Agent, expected: LoopState, node: LoopState['node'], operation: () => Promise<void> | void): Promise<boolean> {
  return withLoopOperationLock(agent, async () => {
    if (!isCurrentExecution(agent, expected, node)) return false
    await operation()
    return true
  })
}

function settleTodos(agent: Agent, completed: boolean): void {
  const latest = [...agent.session.events].reverse().find(event => event.type === 'todo/write')
  if (latest?.type !== 'todo/write') return
  const todos = completed
    ? latest.data.todos.map(todo => ({ ...todo, status: 'completed' as const }))
    : []
  agent.session.append('todo/write', { todos })
}

function parseStart(rawInput: string, defaultMaxAttempts: number): { goal: string; maxAttempts: number; acceptance: AcceptanceContract } {
  const trimmed = rawInput.trim()
  if (!trimmed) throw new Error('usage: /loop start <goal>')
  if (trimmed.startsWith('{')) {
    const input = JSON.parse(trimmed) as { goal?: string; maxAttempts?: number; acceptance?: AcceptanceContract }
    if (!input.goal) throw new Error('start payload requires goal')
...
function workflowPrompt(state: LoopState): string {
  return [
...
function assistantText(event: { data: unknown }): string {
...
function resultMarker(text: string): { status: 'pass' | 'fail'; summary: string } | undefined {
  const match = text.match(/LOOPGRAPH_RESULT:\s*(\{.*\})/s)
  if (!match) return undefined
  try {
    const value = JSON.parse(match[1]) as { status?: 'pass' | 'fail'; summary?: string }
...
async function runDoublecheckGate(ctx: Context, agent: Agent, signal: AbortSignal): Promise<{ passed: boolean; text: string }> {
  const command = ctx.commands.find(agent, 'gate')
  if (!command) return { passed: false, text: 'dsh-doublecheck is required but not mounted' }
...
async function handleAssistantResult(ctx: Context, config: Config, agent: Agent, state: LoopState, text: string, signal: AbortSignal): Promise<void> {
  ...
  await applyToCurrentExecution(agent, verified, roleNode(verified, 'verify'), async () => {
    ...
    if (await gitCandidateFingerprint(cwd) !== prepared.fingerprint) throw new Error('candidate changed after verification; refusing automatic promotion')
...
function loopLogs(agent: Agent, limit = 20): unknown[] {
  return loadLoopEvents(agent)
    ...
    .map((payload, index) => {
      ...
      if (typeof evidence.output === 'string' && evidence.output.length > 1000) evidence.output = `${evidence.output.slice(0, 1000)}...`
...
export function apply(ctx: Context, config: Config): void {
  ...
  const recover = async (agent: Agent, current: LoopState, action: string, signal = new AbortController().signal): Promise<CommandResult> => {
    ...
    if (action === 'verify-existing') {
      let running: LoopState | undefined
  ...
  const promptRecovery = async (agent: Agent, state: LoopState): Promise<void> => {
    try {
      ...
      const actions: Record<string, string> = { '验证现状': 'verify-existing', '同 Token 重试': 'retry-same-attempt', '恢复 Baseline': 'restore-baseline', '终止并保留现场': 'abort-preserve' }
      if (selected && actions[selected]) await recover(agent, getState(agent) ?? state, actions[selected])
    } catch (error) {
  ...
  ctx.on('agent/created', async ({ agent }: { agent: Agent }) => {
    let uncertain: LoopState | undefined
    await withLoopOperationLock(agent, async () => {
  ...
  ctx.on('session/event', async (session, event) => {
    ...
    if (!event) return
    const state = getState(agent)
    if (!state || state.status !== 'RUNNING' || event.type !== 'assistant/message') return
  ...
  ctx.commands.register({
    ...
    handler: async ({ agent, rawInput, signal }): Promise<CommandResult> => {
      ...
      const current = getState(agent)
      if (action === 'start') {
        if (config.workflowName) return { kind: 'error', text: `configured workflow ${config.workflowName} is disabled until terminal outcome, retry, pause/resume, restart reconciliation, and workspace adoption are implemented` }
        return withLoopOperationLock(agent, async () => {
          ...
          const cwd = agent.session.header.cwd
          if (!cwd) return { kind: 'error', text: 'loop start requires a Git workspace' }
          let baseline: string
      ...
      const locked = (operation: (state: LoopState) => Promise<CommandResult>): Promise<CommandResult> => withLoopOperationLock(agent, async () => {
        const state = getState(agent)
        return state ? operation(state) : { kind: 'error', text: 'no active loop workflow' }
      })
      if (action === 'status') {
        if (current.status === 'COMPLETED') settleTodos(agent, true)
        if (current.status === 'FAILED') settleTodos(agent, false)
      ...
      if (action === 'explain') return { kind: 'success', text: JSON.stringify({ state: getState(agent), recentEvents: loopLogs(agent, 20) }, null, 2) }
      if (action === 'pause') {
        return locked(async state => {
          if (state.status !== 'RUNNING') return { kind: 'error', text: 'pause requires RUNNING status' }
      ...
      if (action === 'resume') {
        return locked(async state => {
          if (state.status !== 'PAUSED') return { kind: 'error', text: 'resume requires PAUSED status' }
      ...
      if (action === 'retry') {
        return locked(async state => {
          if (state.status !== 'WAITING_HITL') return { kind: 'error', text: 'retry requires WAITING_HITL status' }
      ...
      if (action === 'recover') {
        if (parts[0] === 'verify-existing') return recover(agent, current, parts[0], signal)
        return locked(state => recover(agent, state, parts[0] ?? '', signal))
      }
      if (action === 'approve') {
        return locked(async state => {
          if (state.status !== 'WAITING_HITL' || state.hitlReason !== 'PROMOTION_REVIEW') return { kind: 'error', text: 'approve requires a verified PROMOTION_REVIEW state' }
          const cwd = agent.session.header.cwd
          if (!cwd) return { kind: 'error', text: 'current session has no Git workspace' }
      ...
      if (action === 'reject') {
        return locked(async state => {
          if (state.status !== 'WAITING_HITL') return { kind: 'error', text: 'reject requires WAITING_HITL status' }
          const cwd = agent.session.header.cwd
          if (!cwd || !state.baselineCommit) return { kind: 'error', text: 'reject cleanup requires a Git workspace and baseline commit' }
          try {
      ...
      if (action === 'rollback') {
        return locked(async state => {
          if (state.status !== 'COMPLETED') return { kind: 'error', text: 'rollback requires COMPLETED status' }
          const versionId = parts[0]
          if (!versionId) return { kind: 'error', text: 'usage: /loop rollback <version-id>' }
          const target = state.versions?.find(version => version.id === versionId)
          if (!target) return { kind: 'error', text: 'rollback target is not owned by this workflow' }
          const cwd = agent.session.header.cwd
