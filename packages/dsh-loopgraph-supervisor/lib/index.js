...
function promotedFields(state, sha) {
    const id = `version:${state.workflowId}:${state.attempt}`;
    const versions = [...(state.versions ?? []), { id, sha, parentId: state.activeVersion, status: 'PROMOTED' }];
...
export function apply(ctx, config) {
    ...
    ctx.on('agent/turn-stopping', async ({ agent, signal }) => {
        const state = getState(agent);
        if (!state || state.status !== 'RUNNING' || state.node !== 'EXECUTE')
            return;
        ...
        handler: async ({ agent, rawInput, signal }) => {
            ...
            if (action === 'start') {
                ...
                return withLoopOperationLock(agent, async () => {
                    ...
                    const state = { ...initialState(workflowId, input.goal, input.maxAttempts, input.acceptance), specRevision: specRevision(input.goal, input.maxAttempts, input.acceptance), baselineCommit: baseline, activeVersion: baselineVersion, versions: [{ id: baselineVersion, sha: baseline, status: 'BASELINE' }] };
            ...
            if (action === 'rollback') {
                return locked(async (state) => {
                    ...
                    try {
                        ...
                        const versions = [...(state.versions ?? []), { id: rollbackId, sha: target.sha, parentId: state.activeVersion, status: 'ROLLED_BACK' }];
