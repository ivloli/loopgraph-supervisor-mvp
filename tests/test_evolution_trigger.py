...
def test_human_can_trigger_rsi_without_waiting_for_failure_threshold():
    trigger = trigger_from_human("coding-supervisor", "reviewer", "Escalate deterministic failure while preserving human_gate")

    assert trigger.source == "human_feedback"
    assert trigger.evidence == (HumanFeedback("reviewer", "Escalate deterministic failure while preserving human_gate"),)


def test_task_and_human_triggers_survive_store_reconstruction():
    store = EvolutionTriggerStore(SQLiteStore(":memory:"))
    trigger = trigger_from_human("coding-supervisor", "reviewer", "Review retry policy")
